...
function isLinkToFragment(path) {
  return path[0] === "#";
}
...
var ObsidianLinksSettingTab = class extends import_obsidian6.PluginSettingTab {
  ...
  display() {
    ...
    const settingOnConvertToMdlinkAppendMdExtension = new import_obsidian6.Setting(containerEl).setName("Append .md extension").addToggle((toggle) => {
    ...
    const showOnConvertToMdLinkkAppendMdExtensionSetting = (show) => {
      if (show) {
    ...
    new import_obsidian6.Setting(containerEl).setName("Append .md file extension").setDesc("Append .md file extension during convertion to a markdown link.").setClass("setting-item-feature1").addToggle((toggle) => {
      toggle.setValue(this.plugin.settings.ffOnConvertToMdlinkAppendMdExtension).onChange(async (value) => {
...
var ConvertToMdlinkCommandBase = class extends CommandBase {
  ...
  async convertLinkToMarkdownLink(linkData, editor, setCursor = true, linkOffset = 0) {
    ...
    if (linkData.type === 8 /* Autolink */ && linkData.destination && RegExPatterns.Email.test(linkData.destination.content)) {
      ...
      if (this.obsidianProxy.settings.ffOnConvertToMdlinkAppendMdExtension && this.obsidianProxy.settings.onConvertToMdlinkAppendMdExtension && !isAbsoluteUri(destination) && !isAbsoluteFilePath(destination) && !isLinkToFragment(destination)) {
        const extRegEx = /(.*?)(\.([^*"\/\<>:|\?]*?))?(#.*)?$/;
